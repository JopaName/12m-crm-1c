...
import { X, Camera, Save, User, Building2, Briefcase, Phone, FileText, Mail } from "lucide-react";
import { useQueryClient, useQuery } from "@tanstack/react-query";

export default function ProfileModal({ user, onClose, profileUserId }: { user: any; onClose: () => void; profileUserId?: string | null }) {
  const queryClient = useQueryClient();
  const isSelf = !profileUserId || profileUserId === user?.id;

  // Load other user's profile if viewing someone else
  const { data: otherUser } = useQuery({
    queryKey: ["user-profile", profileUserId],
    queryFn: () => authAPI.getUserById(profileUserId!),
    enabled: !!profileUserId && !isSelf,
  });

  const profile = isSelf ? user : (otherUser || user);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="relative bg-gradient-to-br from-primary-500 to-primary-700 px-6 pt-8 pb-6">
          ...
          <div className="flex flex-col items-center">
            {/* Avatar */}
            <div className={"relative mb-3" + (isSelf ? " group" : "")}>
              {avatar ? (
            ...
            <h2 className="text-lg font-bold text-white">{profile?.firstName} {profile?.lastName}</h2>
            <p className="text-sm text-white/70 flex items-center gap-1"><Mail className="w-3 h-3" />{profile?.email}</p>
            <span className="mt-1 px-2 py-0.5 bg-white/20 rounded-full text-[10px] text-white font-medium">{profile?.role?.name || ""}</span>
          </div>
        </div>
        ...
        <div className="px-6 py-4 space-y-3">
          ...
          <div>
            ...
            {isSelf ? (
              ...
              <p className="text-sm text-gray-800 py-2">{profile?.phone || "—"}</p>
            )}
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              {isSelf ? (
                ...
                <p className="text-sm text-gray-800 py-2">{profile?.department || "—"}</p>
              )}
            </div>
            <div>
              ...
              {isSelf ? (
                ...
                <p className="text-sm text-gray-800 py-2">{profile?.position || "—"}</p>
              )}
            </div>
          </div>
          <div>
            ...
            {isSelf ? (
              ...
              <p className="text-sm text-gray-700 py-2">{profile?.bio || "Информация не заполнена"}</p>
            )}
          </div>
        </div>
