...
export default function WarehousePage() {
  ...
  const [itemForm, setItemForm] = useState({ productName: "", quantity: "0", unit: "шт", note: "" });

  const { data: categories } = useQuery({
    queryKey: ["warehouse-categories"],
    queryFn: () => warehouseAPI.getCategories().then((r) => r.data),
  });

  const { data: items } = useQuery({
    queryKey: ["warehouse-items", selectedCategory],
    queryFn: () => selectedCategory ? warehouseAPI.getCategoryItems(selectedCategory).then((r) => r.data) : Promise.resolve([]),
  ...
  const createItem = useMutation({
    ...
    onSuccess: () => {
      ...
      setItemForm({ productName: "", quantity: "0", unit: "шт", note: "" });
    },
              ...
              <button
                onClick={() => {
                  if (editingItem) {
                    ...
                    createItem.mutate({ categoryId: selectedCategory, data: itemForm });
                  }
                }}
                className="w-full py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm"
              >
          ...
          <div className="p-2">
            {rootCategories.map((cat: any) => (
              <CategoryNode
                ...
                onDelete={(id) => { if (confirm("Удалить каталог и все товары?")) deleteCategory.mutate(id); }}
              />
            ))}
            {(!categories || categories.length === 0) && (
              <p className="text-sm text-gray-400 text-center py-4">Нет каталогов</p>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100">
          <div className="p-4 border-b border-gray-100 flex items-center justify-between">
            <div>
              ...
              {selectedCat && (
                <span className="text-sm text-gray-400 ml-2">({items?.length || 0} товаров)</span>
              )}
            </div>
            {selectedCategory && (
              <button
                onClick={() => { setEditingItem(null); setItemForm({ productName: "", quantity: "0", unit: "шт", note: "" }); setShowItemForm(true); }}
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="text-left px-3 py-2 text-xs font-medium text-gray-500">Ед.</th>
                    <th className="text-left px-3 py-2 text-xs font-medium text-gray-500">Прим.</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {items.map((item: any) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      <td className="px-3 py-2.5 text-sm font-medium">{item.productName}</td>
                      <td className="px-3 py-2.5 text-sm">{item.quantity}</td>
                      <td className="px-3 py-2.5 text-sm text-gray-500">{item.unit}</td>
                      <td className="px-3 py-2.5 text-sm text-gray-400">{item.note || "-"}</td>
                      <td className="px-3 py-2.5 text-right">
                        <button
                          onClick={() => { setEditingItem(item); setItemForm({ productName: item.productName, quantity: String(item.quantity), unit: item.unit, note: item.note || "" }); setShowItemForm(true); }}
                          className="p-1 hover:bg-blue-100 rounded text-blue-500 mr-1"
                        >
                          ✏️
                        </button>
                        <button
                          onClick={() => { if (confirm("Удалить товар?")) deleteItem.mutate(item.id); }}
...
function CategoryNode({ category, allCategories, selectedId, onSelect, onDelete }: {
  category: any;
  allCategories: any[];
  ...
  return (
    <div>
      <div
        className={`flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer mb-1 ${isSelected ? "bg-blue-100 text-blue-700" : "hover:bg-gray-50"}`}
        onClick={() => onSelect(category.id)}
      >
        <div className="flex items-center gap-2">
          {children.length > 0 && (
            <button onClick={(e) => { e.stopPropagation(); setExpanded(!expanded); }} className="text-xs text-gray-400">
              {expanded ? "▼" : "▶"}
            </button>
          )}
        ...
        <button
          ...
          className="p-1 hover:bg-red-100 rounded text-red-400 opacity-0 hover:opacity-100 text-xs"
        >
          ✕
        </button>
      </div>
